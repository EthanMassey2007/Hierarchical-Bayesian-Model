...
BASE_DIR = os.environ.get("HBM_PROJECT_DIR")
if BASE_DIR:
    BASE_DIR = os.path.abspath(BASE_DIR)
else:
    candidate = SCRIPT_DIR
    while candidate and candidate != os.path.dirname(candidate):
        if os.path.exists(os.path.join(candidate, "data", "complete_combined_datasets.csv")):
            BASE_DIR = candidate
            break
        candidate = os.path.dirname(candidate)
    if BASE_DIR is None:
        raise FileNotFoundError("Could not find project root. Set HBM_PROJECT_DIR to the project directory.")
...
def run_train_test_evaluation(df: pd.DataFrame):
    ...
    if SAVE_OUTPUTS:
        ...
        metrics_path = os.path.join(OUTPUT_DIR, "base_model_interpolation_train_test_metrics.csv")
        predictions_path = os.path.join(OUTPUT_DIR, "base_model_interpolation_test_predictions.csv")
...
def main():
    ...
    if SAVE_OUTPUTS:
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        metrics_path = os.path.join(OUTPUT_DIR, "base_model_interpolation_metrics.csv")
        predictions_path = os.path.join(OUTPUT_DIR, "base_model_interpolation_predictions.csv")
