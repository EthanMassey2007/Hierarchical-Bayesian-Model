...
def run_train_test_evaluation(df: pd.DataFrame):
    ...
    if SAVE_OUTPUTS:
        ...
        metrics_path = os.path.join(
            OUTPUT_DIR,
            "base_model_lag_cases_weather_train_test_metrics.csv",
        )
        predictions_path = os.path.join(
            OUTPUT_DIR,
            "base_model_lag_cases_weather_test_predictions.csv",
        )
...
def main():
    ...
    if SAVE_OUTPUTS:
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        metrics_path = os.path.join(OUTPUT_DIR, "base_model_lag_cases_weather_metrics.csv")
        predictions_path = os.path.join(
            OUTPUT_DIR,
            "base_model_lag_cases_weather_predictions.csv",
        )
    ...
    if MAKE_PLOTS:
        ...
        plt.title("Lagged-cases + lagged-weather hierarchical model: actual vs predicted")
        plt.tight_layout()
        plt.show()
