# =========================================================
# S6: S2 + rainfall-by-region interaction using R-INLA
# =========================================================
...
# S6 additionally tests whether the rainfall association differs by current
...
RAINFALL_REGION_REFERENCE <- "Rio de Janeiro"
REGION_FIELD <- "regiao_intermediaria_nome"
REGION_INTERACTION_PREFIX <- "rainfall_lag_region"
...
FULL_PREDICTIONS_CSV <- file.path(OUTPUT_DIR, "spatial_inla_s6_rainfall_region_full_predictions.csv")
...
set_rainfall_region_interaction_metadata <- function(region_levels) {
  region_levels <- sort(unique(region_levels))
  if (!(RAINFALL_REGION_REFERENCE %in% region_levels)) {
    stop(sprintf(
      "Reference region '%s' is not present. Available regions: %s",
      RAINFALL_REGION_REFERENCE,
...
add_rainfall_region_interactions <- function(df) {
  if (!("rainfall_lag_z" %in% names(df))) {
    stop("rainfall_lag_z must exist before creating rainfall-region interactions.")
  }
  ...
  for (i in seq_along(interaction_regions)) {
    ...
    df[, (col) := rainfall_lag_z * as.integer(region == region_name)]
  }
...
build_model_dataframe <- function() {
  ...
  interaction_info <- set_rainfall_region_interaction_metadata(df$region)

  rows_before_drop <- nrow(df)
  df <- df[complete.cases(df[, ..BASE_COVARIATES])]

  cat("M5/S6 rainfall-region dataframe built\n")
  cat("Original rows:", original_rows, "\n")
  cat("Model rows:", nrow(df), "\n")
  cat("Dropped missing S6 covariates:", rows_before_drop - nrow(df), "\n")
  cat("Municipios:", uniqueN(df$municipio), "\n")
  cat("Regions:", paste(interaction_info$region_levels, collapse = ", "), "\n")
  cat("Rainfall-region reference:", RAINFALL_REGION_REFERENCE, "\n")
  cat("Rainfall-region interaction columns:", paste(REGION_INTERACTION_COVARIATES, collapse = ", "), "\n")
...
main <- function() {
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    test_dt <- scaled$test
    train_dt <- add_rainfall_region_interactions(train_dt)
    test_dt <- add_rainfall_region_interactions(test_dt)
    ...
    cat("\nS6 train-fit fixed effects:\n")
    print(train_fit$summary.fixed)

    train_region_rainfall <- summarize_region_rainfall_effects(train_fit)
    cat("\nS6 train-fit region-specific rainfall effects:\n")
    print(train_region_rainfall)

    if (SAVE_OUTPUTS) {
      dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
      fwrite(metrics, file.path(OUTPUT_DIR, "spatial_inla_s6_rainfall_region_train_test_metrics.csv"))
      fwrite(full_region_rainfall, file.path(OUTPUT_DIR, "spatial_inla_s6_rainfall_region_full_effects.csv"))
      fwrite(train_region_rainfall, file.path(OUTPUT_DIR, "spatial_inla_s6_rainfall_region_train_effects.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(OUTPUT_DIR, "spatial_inla_s6_rainfall_region_test_predictions.csv"))
    }
  }
}
