...
build_model_dataframe <- function() {
  ...
  cat("M5/S1 dataframe built\n")
  cat("Original rows:", original_rows, "\n")
  cat("Model rows:", nrow(df), "\n")
  cat("Dropped missing M5 covariates:", rows_before_drop - nrow(df), "\n")
...
main <- function() {
  ...
  cat("\nS1 full-data model criteria:\n")
  print(criteria)
  dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
  fwrite(criteria, file.path(OUTPUT_DIR, "spatial_inla_s1_model_criteria.csv"))
  cat("Criteria:", file.path(OUTPUT_DIR, "spatial_inla_s1_model_criteria.csv"), "\n")
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    cat("No-leakage policy: test lagged cases must come from before test start.\n")

    cat("\nS1 train/test metrics:\n")
    print(metrics)

    cat("\nS1 train-fit fixed effects:\n")
