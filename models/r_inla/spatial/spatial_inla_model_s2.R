...
main <- function() {
  ...
  fwrite(criteria, file.path(OUTPUT_DIR, "spatial_inla_s2_model_criteria.csv"))
  cat("Criteria:", file.path(OUTPUT_DIR, "spatial_inla_s2_model_criteria.csv"), "\n")
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    if (SAVE_OUTPUTS) {
      dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
      fwrite(metrics, file.path(OUTPUT_DIR, "spatial_inla_s2_train_test_metrics.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(OUTPUT_DIR, "spatial_inla_s2_test_predictions.csv"))
    }
  }
}
