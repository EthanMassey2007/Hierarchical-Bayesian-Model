...
# S7 additionally tests whether the temperature association differs by current
# IBGE intermediate geographic region of Rio de Janeiro.
...
TEMPERATURE_REGION_REFERENCE <- "Rio de Janeiro"
...
build_region_lookup <- function() {
  regions <- fread(REGION_LOOKUP_FILE)
  regions <- clean_columns(regions)

  required <- c("ibge_code", REGION_FIELD)
  missing_cols <- setdiff(required, names(regions))
  if (length(missing_cols) > 0) {
    stop(sprintf(
      "rj_ibge_intermediate_regions.csv is missing region columns: %s",
      paste(missing_cols, collapse = ", ")
    ))
  }

  regions[, ibge_code := as.integer(ibge_code)]
  regions[, region := as.character(get(REGION_FIELD))]
  region_lookup <- unique(regions[!is.na(ibge_code) & !is.na(region), .(ibge_code, region)])
  region_lookup <- region_lookup[!duplicated(ibge_code)]

  if (nrow(region_lookup) == 0) {
    stop("No RJ intermediate region lookup entries found in rj_ibge_intermediate_regions.csv.")
  }
