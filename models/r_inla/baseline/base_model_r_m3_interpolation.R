# =========================================================
# R-M3: non-spatial same-week covariate interpolation model using R-INLA
# =========================================================
# Converted from base_model_interpolation.py.
# Uses same-week rainfall, IDHM, and interpolated humidity/temperature.
...
MODEL_ID <- "R_M3"
MODEL_DESCRIPTION <- "base + same-week covariates + humidity/temperature interpolation"
...
INTERPOLATED_COVARIATES <- c("humidity", "temperature")
...
CRITERIA_CSV <- file.path(OUTPUT_DIR, "r_m3_model_criteria.csv")
METRICS_CSV <- file.path(OUTPUT_DIR, "r_m3_train_test_metrics.csv")
FIXED_EFFECTS_CSV <- file.path(OUTPUT_DIR, "r_m3_fixed_effects.csv")
...
interpolate_internal_gaps <- function(values, dates, max_gap_weeks) {
  values <- as.numeric(values)
  dates_numeric <- as.numeric(as.Date(dates))
  ...
  missing_runs <- run_dt[missing == TRUE, .(
    start_idx = min(idx),
    end_idx = max(idx),
    n_missing = .N
  ), by = run_id]
  ...
  filled <- values
  for (row in seq_len(nrow(missing_runs))) {
    start_idx <- missing_runs$start_idx[row]
    end_idx <- missing_runs$end_idx[row]
    previous_idx <- start_idx - 1L
    next_idx <- end_idx + 1L

    has_bounding_values <- previous_idx >= 1L &&
      next_idx <= length(values) &&
      !is.na(values[previous_idx]) &&
      !is.na(values[next_idx])

    if (has_bounding_values && missing_runs$n_missing[row] <= max_gap_weeks) {
      filled[start_idx:end_idx] <- interpolated[start_idx:end_idx]
    }
  }
...
interpolate_weather_covariates <- function(df, label) {
  setorder(df, municipio, date)
  missing_before <- df[, lapply(.SD, function(x) sum(is.na(x))), .SDcols = INTERPOLATED_COVARIATES]

  for (col in INTERPOLATED_COVARIATES) {
    df[, (col) := interpolate_internal_gaps(get(col), date, INTERPOLATION_LIMIT_WEEKS), by = municipio]
  }

  df[, humidity := pmin(pmax(humidity, 0), 100)]
  df[, temperature := pmin(pmax(temperature, 0), 45)]

  missing_after <- df[, lapply(.SD, function(x) sum(is.na(x))), .SDcols = INTERPOLATED_COVARIATES]
  cat(label, "interpolation filled values:\n")
  for (col in INTERPOLATED_COVARIATES) {
    filled <- missing_before[[col]] - missing_after[[col]]
    cat(" ", col, ": filled ", filled, ", remaining missing ", missing_after[[col]], "\n", sep = "")
  }

  df
}

build_model_dataframe <- function(apply_interpolation = TRUE) {
  ...
  if (apply_interpolation) {
    df <- interpolate_weather_covariates(df, "full-data")
  }
  ...
  if (apply_interpolation && length(BASE_COVARIATES) > 0) {
  ...
  if (apply_interpolation) {
    ...
    cat("Interpolation deferred; missing covariates retained for split-specific handling.\n")
  }
...
main <- function() {
  ...
  train_dt <- evaluation_df[year >= TRAIN_START_YEAR & year <= TRAIN_END_YEAR]
  test_dt <- evaluation_df[year >= TEST_START_YEAR & year <= TEST_END_YEAR]
  ...
  train_dt <- interpolate_weather_covariates(train_dt, "train")
  test_dt <- interpolate_weather_covariates(test_dt, "test")
  train_dt <- train_dt[complete.cases(train_dt[, ..BASE_COVARIATES])]
  test_dt <- test_dt[complete.cases(test_dt[, ..BASE_COVARIATES])]
  ...
  cat("No-leakage policy: interpolation is applied separately to train and test.\n")
  if (USE_CASE_LAG) {
