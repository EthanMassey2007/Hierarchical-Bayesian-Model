...
def build_municipio_lookup() -> pd.DataFrame:
    ...
    fallback_name = hub["nome do município"].apply(normalize_name)
    hub.loc[~hub["municipio"].apply(valid_municipio_name), "municipio"] = fallback_name
    hub["ibge_code"] = pd.to_numeric(hub["co_ibge"], errors="coerce")
    hub_lookup = (
        hub[["municipio", "ibge_code"]]
        .dropna()
        .loc[lambda d: d["municipio"].apply(valid_municipio_name)]
        .drop_duplicates()
        .assign(ibge_code=lambda d: d["ibge_code"].astype(int))
    )
