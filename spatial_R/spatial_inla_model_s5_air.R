# =========================================================
# S5-air: S2 + air mobility using R-INLA
# =========================================================
...
#   4. air_passenger_week_lag_log, a previous-month air passenger mobility covariate.
...
AIR_FILE <- file.path(DATA_DIR, "aero_anac_2017_2023.parquet")
...
month_start_date <- function(date) {
  as.Date(sprintf("%s-01", format(date, "%Y-%m")))
}

previous_month_start <- function(date) {
  current <- month_start_date(date)
  as.Date(format(current - 1L, "%Y-%m-01"))
}
...
# Air mobility helper
# =========================================================
build_air_mobility <- function(model_ibge_codes) {
  air <- as.data.table(read_parquet(AIR_FILE))
  ...
  missing_cols <- setdiff(required_cols, names(air))
  if (length(missing_cols) > 0) {
    stop(sprintf(
      "aero_anac_2017_2023.parquet is missing required columns: %s",
      paste(missing_cols, collapse = ", ")
    ))
  }

  model_ibge_codes <- as.integer(model_ibge_codes)
  air[, year := suppressWarnings(as.integer(ano))]
  air[, month := suppressWarnings(as.integer(mes))]
  air[, co_muni_ori := suppressWarnings(as.integer(co_muni_ori))]
  air[, co_muni_des := suppressWarnings(as.integer(co_muni_des))]
  air[, aero_pass_week := suppressWarnings(as.numeric(aero_pass_week))]
  air <- air[
    year >= DATA_START_YEAR &
      ...
      !is.na(co_muni_ori) &
      !is.na(co_muni_des) &
      !is.na(aero_pass_week) &
      is.finite(aero_pass_week) &
      aero_pass_week > 0
  ]

  outbound <- air[
    co_muni_ori %in% model_ibge_codes,
    .(ibge_code = co_muni_ori, year, month, air_passenger_week_lag_raw = aero_pass_week)
  ]
  inbound <- air[
    co_muni_des %in% model_ibge_codes,
    .(ibge_code = co_muni_des, year, month, air_passenger_week_lag_raw = aero_pass_week)
  ]

  air_strength <- rbindlist(
    list(outbound, inbound),
    use.names = TRUE
  )[
    ,
    .(air_passenger_week_lag_raw = sum(air_passenger_week_lag_raw)),
    by = .(ibge_code, year, month)
  ]

  air_strength[, air_source_month := as.Date(sprintf("%04d-%02d-01", year, month))]
  air_strength[, c("year", "month") := NULL]
  air_strength[, air_passenger_week_lag_log := log1p(air_passenger_week_lag_raw)]

  if (any(is.na(air_strength$air_passenger_week_lag_log)) ||
      any(!is.finite(air_strength$air_passenger_week_lag_log))) {
    stop("Air mobility aggregation produced invalid values.")
  }
  if (nrow(air_strength) == 0 || sd(air_strength$air_passenger_week_lag_log) == 0) {
    stop("Air mobility has zero variance across model municipality-months.")
  }
...
# =========================================================
# M5 + spatial-lag + air-mobility feature construction
# =========================================================
build_model_dataframe <- function() {
  ...
  air_strength <- build_air_mobility(sort(unique(df$ibge_code)))
  df[, air_source_month := previous_month_start(date)]
  df <- merge(df, air_strength, by = c("ibge_code", "air_source_month"), all.x = TRUE, sort = FALSE)
  df[is.na(air_passenger_week_lag_raw), air_passenger_week_lag_raw := 0]
  df[is.na(air_passenger_week_lag_log), air_passenger_week_lag_log := 0]
  if (any(is.na(df$air_passenger_week_lag_log))) {
    stop("Air mobility is missing for one or more model rows.")
  }
  bad_air_lag <- df[air_source_month >= month_start_date(date)]
  if (nrow(bad_air_lag) > 0) {
    stop("Air mobility lag leakage check failed.")
  }
  ...
  cat("M5/S5-air dataframe built\n")
  cat("Original rows:", original_rows, "\n")
  cat("Model rows:", nrow(df), "\n")
  cat("Dropped missing S5-air covariates:", rows_before_drop - nrow(df), "\n")
  ...
  cat("Rows with zero previous-month air mobility:", sum(df$air_passenger_week_lag_raw == 0), "\n")
...
main <- function() {
  ...
  cat("\nS5-air full-data fixed effects:\n")
  ...
  cat("\nS5-air full-data model criteria:\n")
  print(criteria)
  dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
  fwrite(criteria, file.path(OUTPUT_DIR, "spatial_inla_s5_air_model_criteria.csv"))
  cat("Criteria:", file.path(OUTPUT_DIR, "spatial_inla_s5_air_model_criteria.csv"), "\n")
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    cat("\nS5-air train/test metrics:\n")
    print(metrics)

    cat("\nS5-air train-fit fixed effects:\n")
    print(train_fit$summary.fixed)

    if (SAVE_OUTPUTS) {
      dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
      fwrite(metrics, file.path(OUTPUT_DIR, "spatial_inla_s5_air_train_test_metrics.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(OUTPUT_DIR, "spatial_inla_s5_air_test_predictions.csv"))
    }
  }
}
