...
# =========================================================
# Fits S6 if needed, extracts the estimated rainfall effect by official IBGE
# mesoregion, joins it to data/RJ.json, and maps the rainfall relative risk.
#
...
S6_SCRIPT <- file.path(PROJECT_DIR, "spatial_R", "spatial_inla_model_s6_rainfall_region.R")
...
plot_rainfall_region_map <- function(effects, region_lookup) {
  ...
  p <- ggplot(map_sf) +
    ...
    labs(
      ...
      caption = "Rainfall-by-IBGE-mesoregion interactions. Values above 1 indicate higher expected cases."
    ) +
