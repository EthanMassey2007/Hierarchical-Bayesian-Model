...
# =========================================================
# Fits S7 if needed, extracts the estimated temperature effect by official IBGE
# mesoregion, joins it to data/RJ.json, and maps the temperature relative risk.
#
...
S7_SCRIPT <- file.path(PROJECT_DIR, "spatial_R", "spatial_inla_model_s7_temperature_region.R")
...
plot_temperature_region_map <- function(effects, region_lookup) {
  ...
  p <- ggplot(map_sf) +
    ...
    labs(
      ...
      caption = "Temperature-by-IBGE-mesoregion interactions. Values above 1 indicate higher expected cases."
    ) +
