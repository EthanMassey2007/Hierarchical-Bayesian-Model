# =========================================================
# S4-road: S2 + road connectivity using R-INLA
# =========================================================
...
#   4. road_connectivity_log, a static municipality-level road connectivity covariate.
...
ROAD_FILE <- file.path(DATA_DIR, "fluvi_road_ibge.parquet")
...
# =========================================================
# Road connectivity helper
# =========================================================
build_road_connectivity <- function(model_ibge_codes) {
  road <- as.data.table(read_parquet(ROAD_FILE))

  required_cols <- c("co_muni_ori", "co_muni_des", "road_conec")
  missing_cols <- setdiff(required_cols, names(road))
  if (length(missing_cols) > 0) {
    stop(sprintf(
      "fluvi_road_ibge.parquet is missing required columns: %s",
      paste(missing_cols, collapse = ", ")
    ))
  }

  model_ibge_codes <- as.integer(model_ibge_codes)
  road[, co_muni_ori := as.integer(co_muni_ori)]
  road[, co_muni_des := as.integer(co_muni_des)]
  road[, road_conec := as.numeric(road_conec)]
  road <- road[
    !is.na(co_muni_ori) &
      !is.na(co_muni_des) &
      !is.na(road_conec) &
      is.finite(road_conec) &
      road_conec > 0
  ]

  # Treat the road network as undirected and collapse repeated pair records.
  # This prevents double counting when an origin-destination pair appears more than once.
  road[, pair_min := pmin(co_muni_ori, co_muni_des)]
  road[, pair_max := pmax(co_muni_ori, co_muni_des)]
  road_edges <- road[
    ,
    .(road_conec = max(road_conec)),
    by = .(pair_min, pair_max)
  ]

  incident_edges <- road_edges[pair_min %in% model_ibge_codes | pair_max %in% model_ibge_codes]
  origin_strength <- incident_edges[
    pair_min %in% model_ibge_codes,
    .(ibge_code = pair_min, road_connectivity_raw = road_conec)
  ]
  destination_strength <- incident_edges[
    pair_max %in% model_ibge_codes,
    .(ibge_code = pair_max, road_connectivity_raw = road_conec)
  ]

  road_strength <- rbindlist(
    list(origin_strength, destination_strength),
    use.names = TRUE
  )[
    ,
    .(road_connectivity_raw = sum(road_connectivity_raw)),
    by = ibge_code
  ]

  road_strength <- merge(
    data.table(ibge_code = model_ibge_codes),
    road_strength,
    by = "ibge_code",
    all.x = TRUE,
    sort = FALSE
  )
  road_strength[is.na(road_connectivity_raw), road_connectivity_raw := 0]
  road_strength[, road_connectivity_log := log1p(road_connectivity_raw)]

  if (any(is.na(road_strength$road_connectivity_log)) ||
      any(!is.finite(road_strength$road_connectivity_log))) {
    stop("Road connectivity aggregation produced invalid values.")
  }
  if (sd(road_strength$road_connectivity_log) == 0) {
    stop("Road connectivity has zero variance across model municipalities.")
  }
...
# =========================================================
# M5 + spatial-lag + road-connectivity feature construction
# =========================================================
build_model_dataframe <- function() {
  ...
  road_strength <- build_road_connectivity(sort(unique(df$ibge_code)))
  df <- merge(df, road_strength, by = "ibge_code", all.x = TRUE, sort = FALSE)
  if (any(is.na(df$road_connectivity_log))) {
    stop("Road connectivity is missing for one or more model rows.")
  }
  ...
  cat("M5/S4-road dataframe built\n")
  cat("Original rows:", original_rows, "\n")
  cat("Model rows:", nrow(df), "\n")
  cat("Dropped missing S4-road covariates:", rows_before_drop - nrow(df), "\n")
  ...
  cat("Municipios with zero road connectivity:", uniqueN(df[road_connectivity_raw == 0, municipio]), "\n")
...
main <- function() {
  ...
  cat("\nS4-road full-data fixed effects:\n")
  ...
  cat("\nS4-road full-data model criteria:\n")
  print(criteria)
  dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
  fwrite(criteria, file.path(OUTPUT_DIR, "spatial_inla_s4_road_model_criteria.csv"))
  cat("Criteria:", file.path(OUTPUT_DIR, "spatial_inla_s4_road_model_criteria.csv"), "\n")
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    cat("\nS4-road train/test metrics:\n")
    print(metrics)

    cat("\nS4-road train-fit fixed effects:\n")
    print(train_fit$summary.fixed)

    if (SAVE_OUTPUTS) {
      dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
      fwrite(metrics, file.path(OUTPUT_DIR, "spatial_inla_s4_road_train_test_metrics.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(OUTPUT_DIR, "spatial_inla_s4_road_test_predictions.csv"))
    }
  }
}
