...
main <- function() {
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    if (SAVE_OUTPUTS) {
      fwrite(metrics, file.path(BASE_DIR, "spatial_inla_s4_road_train_test_metrics.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(BASE_DIR, "spatial_inla_s4_road_test_predictions.csv"))
    }
  }
}
