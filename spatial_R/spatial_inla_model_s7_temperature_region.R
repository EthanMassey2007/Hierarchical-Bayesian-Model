...
# S7 additionally tests whether the temperature association differs by official
# IBGE mesoregion of Rio de Janeiro.
...
TEMPERATURE_REGION_REFERENCE <- "Metropolitana do Rio de Janeiro"
REGION_FIELD <- "nome_mesorregião"
REGION_INTERACTION_PREFIX <- "temperature_lag_region"
REGION_INTERACTION_COVARIATES <- character()
REGION_LEVELS <- character()
...
build_region_lookup <- function() {
  ...
  required <- c("co_ibge", REGION_FIELD)
  missing_cols <- setdiff(required, names(hub))
  if (length(missing_cols) > 0) {
    stop(sprintf("hub_pop_density.csv is missing region columns: %s", paste(missing_cols, collapse = ", ")))
  }

  if ("uf" %in% names(hub)) {
    hub <- hub[toupper(as.character(uf)) == "RJ" | co_uf == 33]
  } else {
    hub <- hub[co_uf == 33]
  }

  hub[, ibge_code := as.integer(co_ibge)]
  hub[, region := as.character(get(REGION_FIELD))]
  region_lookup <- unique(hub[!is.na(ibge_code) & !is.na(region), .(ibge_code, region)])
  region_lookup <- region_lookup[!duplicated(ibge_code)]

  if (nrow(region_lookup) == 0) {
    stop("No RJ region lookup entries found in hub_pop_density.csv.")
  }

  region_lookup
}

set_temperature_region_interaction_metadata <- function(region_levels) {
  region_levels <- sort(unique(region_levels))
  if (!(TEMPERATURE_REGION_REFERENCE %in% region_levels)) {
    stop(sprintf(
      "Reference region '%s' is not present. Available regions: %s",
      TEMPERATURE_REGION_REFERENCE,
      paste(region_levels, collapse = ", ")
    ))
  }

  interaction_regions <- setdiff(region_levels, TEMPERATURE_REGION_REFERENCE)
  interaction_cols <- paste0(REGION_INTERACTION_PREFIX, "_", make.names(interaction_regions))

  REGION_LEVELS <<- region_levels
  REGION_INTERACTION_COVARIATES <<- interaction_cols

  list(interaction_cols = interaction_cols, region_levels = region_levels)
}

add_temperature_region_interactions <- function(df) {
  if (!("temperature_lag_z" %in% names(df))) {
    stop("temperature_lag_z must exist before creating temperature-region interactions.")
  }

  interaction_regions <- setdiff(REGION_LEVELS, TEMPERATURE_REGION_REFERENCE)

  for (i in seq_along(interaction_regions)) {
    region_name <- interaction_regions[i]
    col <- REGION_INTERACTION_COVARIATES[i]
    df[, (col) := temperature_lag_z * as.integer(region == region_name)]
  }
...
build_model_dataframe <- function() {
  ...
  df <- merge(df, region_lookup, by = "ibge_code", all.x = TRUE)
  missing_region <- sort(unique(df[is.na(region), municipio]))
  if (length(missing_region) > 0) {
    stop(sprintf(
      "Municipios missing region lookup: %s",
  ...
  interaction_info <- set_temperature_region_interaction_metadata(df$region)

  rows_before_drop <- nrow(df)
  df <- df[complete.cases(df[, ..BASE_COVARIATES])]

  cat("M5/S7 temperature-region dataframe built\n")
  cat("Original rows:", original_rows, "\n")
  cat("Model rows:", nrow(df), "\n")
  cat("Dropped missing S7 covariates:", rows_before_drop - nrow(df), "\n")
  cat("Municipios:", uniqueN(df$municipio), "\n")
  cat("Regions:", paste(interaction_info$region_levels, collapse = ", "), "\n")
  cat("Temperature-region reference:", TEMPERATURE_REGION_REFERENCE, "\n")
  cat("Temperature-region interaction columns:", paste(REGION_INTERACTION_COVARIATES, collapse = ", "), "\n")
...
build_inla_formula <- function(graph_file) {
  ...
  fixed_covariates <- c(z_covariates, REGION_INTERACTION_COVARIATES)
...
summarize_region_temperature_effects <- function(fit) {
  fixed <- as.data.table(fit$summary.fixed, keep.rownames = "term")
  temperature <- fixed[term == "temperature_lag_z"]
  ...
  for (i in seq_along(interaction_regions)) {
    region_name <- interaction_regions[i]
    interaction_term <- REGION_INTERACTION_COVARIATES[i]
    interaction <- fixed[term == interaction_term]
    if (nrow(interaction) != 1) {
...
main <- function() {
  ...
  if (RUN_TRAIN_TEST_EVALUATION) {
    ...
    test_dt <- scaled$test
    train_dt <- add_temperature_region_interactions(train_dt)
    test_dt <- add_temperature_region_interactions(test_dt)
    ...
    cat("\nS7 train-fit fixed effects:\n")
    print(train_fit$summary.fixed)

    train_region_temperature <- summarize_region_temperature_effects(train_fit)
    cat("\nS7 train-fit region-specific temperature effects:\n")
    print(train_region_temperature)

    if (SAVE_OUTPUTS) {
      dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)
      fwrite(metrics, file.path(OUTPUT_DIR, "spatial_inla_s7_temperature_region_train_test_metrics.csv"))
      fwrite(full_region_temperature, file.path(OUTPUT_DIR, "spatial_inla_s7_temperature_region_full_effects.csv"))
      fwrite(train_region_temperature, file.path(OUTPUT_DIR, "spatial_inla_s7_temperature_region_train_effects.csv"))
      output <- copy(test_dt[, .(municipio, year, week, date, cases)])
      output[, predicted_cases := test_pred]
      fwrite(output, file.path(OUTPUT_DIR, "spatial_inla_s7_temperature_region_test_predictions.csv"))
    }
  }
}
